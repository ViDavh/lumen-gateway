...
namespace Vidavh\Gateway\Gateways\Zarinpal;

use Vidavh\Gateway\Constants;
use Vidavh\Gateway\Contracts\PortInterface;
use SoapClient;
use Vidavh\Gateway\Port;


class Zarinpal extends Port implements PortInterface
{
    /**
    ...
    {
        switch ($this->config->get('gateway.gateways.zarinpal.type')) {
            case 'zarin-gate':
                return redirect(str_replace('$Authority', $this->refId, $this->zarinGateUrl));
                break;
            case 'sandbox':
                return redirect($this->sandboxGateUrl. $this->refId);
                break;
            case 'normal':
            default:
    ...
    {
        if (!$this->callbackUrl)
            $this->callbackUrl = $this->config->get('gateway.gateways.zarinpal.callback-url');
    ...
    protected function sendPayRequest()
    {
        ...
        $fields = array(
            'MerchantID' => $this->config->get('gateway.gateways.zarinpal.merchant-id'),
            'Amount' => $this->amount,
            'CallbackURL' => $this->getCallback(),
            'Description' => $this->description ? $this->description : $this->config->get('gateway.gateways.zarinpal.description', ''),
            'Email' => $this->email ? $this->email :$this->config->get('gateway.gateways.zarinpal.email', ''),
            'Mobile' => $this->mobileNumber ? $this->mobileNumber : $this->config->get('gateway.gateways.zarinpal.mobile', ''),
        );
    ...
    {
        $server = $this->config->get('gateway.gateways.zarinpal.server', 'germany');
        switch ($server) {
            case 'iran':
